...
BEGIN
    OPEN v_cards_cursor FOR
        SELECT memory,
         ...
         WHERE c.username = sys_context('USERENV', 'CURRENT_USER')
      GROUP BY memory
      ORDER BY memory ASC;
...
-- open cursor
    OPEN list_vocab FOR SELECT
                            ...
                            FROM
                                ...
                                WHERE COL.username = sys_context('USERENV', 'CURRENT_USER')
                                ;
  -- Return the cursor
...
CREATE OR REPLACE FUNCTION select_inboxes RETURN SYS_REFCURSOR IS
    v_cursor SYS_REFCURSOR;
...
BEGIN
    ...
    OPEN v_cursor FOR
        SELECT t.*
         ...
         WHERE p.username = v_username AND p.name = 'Inbox';
