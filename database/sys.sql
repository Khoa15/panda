...
IS
    ...
    count_user NUMBER := 0;
BEGIN
    v_fullname := UPPER(fullname);
    generated_username := v_fullname;
    
    IF length(v_fullname) - length(replace(v_fullname, ' ', '')) + 1 > 1 THEN    
        generated_username := '';
        FOR i IN 1..LENGTH(v_fullname) LOOP
            IF i = 1 OR SUBSTR(v_fullname, i - 1, 1) = ' ' THEN
                generated_username := generated_username || UPPER(SUBSTR(v_fullname, i, 1));
            END IF;
        END LOOP;
    END IF;
...
--SELECT * FROM CITIES;
--/
---- buoc 1 create directory 
--CREATE DIRECTORY BEAUTIFUL_PLACES_PICTURES1 AS 'D:\HK6\DA_ORACLE\BEAUTIFUL PLACES PICTURES1';
---- BUOC 2 GAN QUYEN (NEU ADMIN KHOI CHAY)
---- GRANT READ,WRITE ON DIRECTORY BEAUTIFUL_PLACES_PICTURES1 TO USER;
--
--SET SERVEROUTPUT ON;
--DECLARE 
--    l_bfile BFILE;
...
--    l_lobmaxsize CONSTANT INTEGER := 65535; -- Gi? tr? c? ??nh thay v? h?ng s?
--BEGIN
--    INSERT INTO ACCOUNT(USERNAME,FULLNAME, AVATAR)
--    VALUES('ABC123','ABC123' ,empty_blob())
--    RETURN AVATAR INTO l_blob;
--    
--    l_bfile := BFILENAME('BEAUTIFUL_PLACES_PICTURES1', 'avt.jpg');
--    DBMS_LOB.fileopen(l_bfile, DBMS_LOB.file_readonly);
--    DBMS_LOB.LOADFROMFILE(l_blob,l_bfile,l_lobmaxsize,l_dest_offset, l_src_offset);
...
--SELECT directory_name, directory_path
--FROM all_directories;
--CREATE DIRECTORY SOUNDS_AND_VOICES AS 'D:\HK6\DA_ORACLE\Sound and Voices';
--/
--DECLARE 
--    f_lob BFILE := BFILENAME('SOUNDS_AND_VOICES', 'AUDIOTEST.wav');
--    b_lob BLOB;
--    length INTEGER;
--BEGIN
--    INSERT INTO ACCOUNT(USERNAME,FULLNAME, RING_TONE)
--    VALUES('ABC12234','ABC12234' ,empty_blob())
--    RETURN RING_TONE INTO b_lob;
